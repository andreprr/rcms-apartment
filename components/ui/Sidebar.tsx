...
export default function Sidebar({ user, onLogout }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-gradient-to-br from-purple-50 via-purple-100 to-lavender-100 flex flex-col h-screen fixed left-0 top-0 border-r border-purple-200/60 shadow-lg">
      {/* Logo */}
      ...
      {/* User Profile */}
      <div className="px-4 py-5 bg-white/50 backdrop-blur-sm border-b border-purple-200/40">
        <div className="flex items-center gap-3 p-3 bg-white/80 rounded-2xl shadow-sm border border-purple-100/60">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-purple-600 font-medium">{user.division}</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 py-4 px-3 overflow-y-auto">
        <div className="text-xs font-bold text-purple-400 uppercase tracking-wider mb-3 px-3">
          Menu
        </div>
        {navItems.map((item) => {
            ...
            <Link
              ...
              className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 mb-1 ${
                isActive
                  ...
                  : 'text-slate-600 hover:bg-purple-100/60 hover:text-purple-700'
              }`}
            >
              {isActive && (
